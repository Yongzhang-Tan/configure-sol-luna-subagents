...
还在担心 Codex 额度不够用？让每一次 Sol 调用都用在关键决策上：由 Sol 负责
规划、复核和最终决策，让 Luna 承担边界清晰的执行任务，在保留回到 Sol 的升级
路径的同时，减少不必要的高阶模型调用。

*实际额度消耗取决于任务、调用次数、账户方案和定价，本项目不承诺固定节省。*
...
只安装 skill 只是让配置器可用，不会应用全局配置。可以使用上面的一句话安装并
执行入口，或在安装后主动调用 `$configure-sol-luna-subagents`，然后新开 Codex
会话或重启客户端。配置加载后，Codex 可以在无需针对每个任务显式提示的情况下，
自动把符合条件的非简单映射和边界清晰的实现任务委派给 Luna。简单任务仍由 Sol
处理，不会强制每个任务都使用子代理。项目 `AGENTS.md`/指令和客户端能力可以
进一步细化或覆盖这一路由。
